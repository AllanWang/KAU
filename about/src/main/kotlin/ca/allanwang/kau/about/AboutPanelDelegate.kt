...
interface AboutPanelContract {
  ...
  /**
   * The base inflation method that will be called for new pages from the page adapter Keep in mind
   * that when inflating, do NOT add the view to the viewgroup Use layoutInflater.inflate(id,
   * parent, false)
   */
  fun inflatePage(activity: AboutActivityBase, parent: ViewGroup, position: Int): View

  /** Convenience method called during [inflatePage] No return value necessary */
  fun onInflatingPage(activity: AboutActivityBase, recycler: RecyclerView, position: Int)

  /** Triggers start of item loading Typically called with [inflatePage] */
  fun loadItems(activity: AboutActivityBase, position: Int)

  /**
   * Called when the [adapter] should take in the items This typically happens once the user has
   * scroll to the page, so they may see a transition
   *
   * [AboutActivityBase.pageStatus] should be updated accordingly, as triggering this does not
   * necessarily mean that the items are added
   */
...
/**
 * Panel delegate for the main page The loading is synchronous, so it is done on inflation All other
 * loading and adding methods are overridden to do nothing There is a
 * [AboutActivityBase.postInflateMainPage] hook that can be overridden to update this panel without
 * extending the whole delegate
 */
...
open class AboutPanelLibs : AboutPanelRecycler() {
  ...
  override fun loadItems(activity: AboutActivityBase, position: Int) {
    ...
    if (activity.pageStatus[position] == 1) addItems(activity, position)
  }

  override fun addItemsImpl(activity: AboutActivityBase, position: Int) {
    with(activity.configs) {
      adapter.add(HeaderIItem(text = libPageTitle, textRes = libPageTitleRes)).add(items)
    }
  }
}
...
open class AboutPanelFaqs : AboutPanelRecycler() {
  ...
  override fun addItemsImpl(activity: AboutActivityBase, position: Int) {
    with(activity.configs) {
      adapter.add(HeaderIItem(text = faqPageTitle, textRes = faqPageTitleRes)).add(items)
    }
  }
}
