...
class MainActivity : KPrefActivity() {
  ...
  override fun onCreateKPrefs(savedInstanceState: Bundle?): KPrefAdapterBuilder.() -> Unit = {
    ...
    checkbox(
        title = R.string.checkbox_1,
        getter = pref::check1,
        setter = { pref.check1 = it },
        builder = { descRes = R.string.desc })
    ...
    checkbox(
        ...
        {
          pref.check2 = it
          reloadByTitle(R.string.checkbox_3)
        })

    /**
     * Since the builder is the last argument and is a lambda, we may write the setup cleanly like
     * so:
     */
    ...
    seekbar(R.string.seekbar, pref::seekbar, { pref.seekbar = it }) {
      ...
      increments = 5
      textViewConfigs = { minEms = 2 }
    }

    subItems(R.string.sub_item, subPrefs()) { descRes = R.string.sub_item_desc }

    plainText(R.string.swipe_showcase) { onClick = { startActivityWithEdge(SWIPE_EDGE_LEFT) } }
    ...
    plainText(R.string.adapter_showcase) {
      onClick = {
        startActivity<AdapterActivity>(
            bundleBuilder = { withSceneTransitionAnimation(this@MainActivity) })
      }
    }

    plainText(R.string.kau_about_app) { onClick = { kauLaunchAbout<AboutActivity>() } }

    header(R.string.long_prefs)

    /**
     * Showcases a little trick. The id for reloading is [R.string.checkbox_2], but the title
     * displayed is still [R.string.checkbox_3]
     */
    checkbox(
        R.string.checkbox_2,
        pref::check2,
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    if (pref.version < BuildConfig.VERSION_CODE) {
      ...
      if (!BuildConfig.DEBUG) showChangelog(R.xml.kau_changelog)
    }
