...
abstract class KPrefItemCore(val core: CoreContract) :
  ...
  protected fun ViewHolder.updateDesc() {
    ...
    if (descRes != INVALID_ID) desc?.visible()?.setText(descRes) else desc?.gone()
  }
  ...
  override fun bindView(holder: ViewHolder, payloads: List<Any>) {
    ...
    with(holder) {
      updateTitle()
      updateDesc()
      if (core.iicon != null) icon?.visible()?.setIcon(core.iicon, 24) else icon?.gone()
      innerFrame?.removeAllViews()
      withTextColor {
        title.setTextColor(it)
        desc?.setTextColor(it.adjustAlpha(0.65f))
      }
      if (buildIsLollipopAndUp) withAccentColor { icon?.drawable?.setTint(it) }
    }
  }
  ...
  interface CoreContract : KPrefItemActions {
    ...
    val titleId: Int
    var titleFun: () -> Int
    @setparam:StringRes var descRes: Int
    var descFun: () -> Int
    var iicon: IIcon?
    var visible: () -> Boolean
  }
