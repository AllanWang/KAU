...
open class KPrefSeekbar(val builder: KPrefSeekbarContract) : KPrefItemBase<Int>(builder) {
  ...
  protected inline val min
    get() = builder.min
  protected inline val max
    get() = builder.max
  protected inline val increment
    get() = builder.increments
  ...
  class KPrefSeekbarBuilder(
    ...
    override var increments: Int = 1
      set(value) {
        if (value <= 0) throw IllegalArgumentException("Seekbar must increment by at least 1")
        field = value
      }
  ...
  /**
   * Helper to convert true value to progress value Progress values start at 0 and increment by 1
   */
