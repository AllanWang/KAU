...
abstract class KPrefActivity : KauBaseActivity(), KPrefActivityContract {
  ...
  val bgCanvas: RippleCanvas
    get() = binding.kauRipple
  val toolbarCanvas: RippleCanvas
    get() = binding.kauToolbarRipple
  val toolbar: Toolbar
  ...
  /**
   * Pops the stack and loads the next kpref list Indices are not checked so ensure that this is
   * possible first
   */
  ...
  /**
   * Reload the current pref list from the stack. This will adjust the list of items change in
   * visibility
   */
  fun reloadList() {
    // If for some reason we are calling a reload before fetching our first kpref list, we will
    // ignore it
  ...
  /**
   * Selectively reload an item based on its index. Note that this might not behave as expected if
   * certain items are not visible, as those items aren't sent to the adapter.
   *
  ...
  /**
   * Iterate through all items and reload if it matches any of the titles If multiple items have the
   * same title, they will all be reloaded
   */
  override fun reloadByTitle(@StringRes vararg title: Int) {
    if (title.isEmpty()) return
    adapter.adapterItems.forEachIndexed { index, item ->
      if (title.any { item.core.titleId == it }) adapter.notifyItemChanged(index)
    }
  }
  ...
  /**
   * Back press handler with status output Returns [true] if the press has been consumed, [false]
   * otherwise
   */
