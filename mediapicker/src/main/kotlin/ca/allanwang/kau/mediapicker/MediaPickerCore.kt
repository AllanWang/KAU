...
abstract class MediaPickerCore<T : GenericItem>(
  ...
  companion object {
    ...
    /**
     * Given the dimensions of our device and a minimum image size, Computer the optimal column
     * count for our grid layout
     *
     * @return column count
     */
    ...
    /**
     * Helper method to retrieve the media from our media picker This is used for both single and
     * multiple photo picks
     */
    fun onMediaPickerResult(resultCode: Int, data: Intent?): List<MediaModel> {
      if (resultCode != Activity.RESULT_OK || data?.hasExtra(MEDIA_PICKER_RESULT) != true) {
        return emptyList()
      }
      return data.getParcelableArrayListExtra(MEDIA_PICKER_RESULT) ?: emptyList()
    }

    /** Number of loaded items we should cache This is arbitrary */
  ...
  /**
   * Request read permissions and load all external items The result will be filtered through {@link
   * #onLoadFinished(Loader, Cursor)} Call this to make sure that we request permissions each time
  ...
  /** Called at the end of [onLoadFinished] when the adapter should add the items */
  ...
  /**
   * Optional filter to decide which items get displayed Defaults to checking their sizes to filter
   * out very small items such as lurking drawables/icons
   *
  ...
  /**
   * Method used to retrieve uri data for API 19+ See <a
