...
/**
 * Singleton to hold our swipe stack All activity pages held with strong references, so it is
 * crucial to call [onDestroy] whenever an activity should be disposed
 */
...
/**
 * The creation binder, which adds the swipe functionality to an activity. Call this during
 * [Activity.onCreate] after all views are added.
 *
 * Preferably, this should be the last line in the onCreate method. Note that this will also capture
 * your statusbar color and nav bar color, so be sure to assign those beforehand if at all.
 *
 ...
 *
 * Don't forget to call [kauSwipeOnCreate] when the activity is created to enable swipe
 * functionality.
 */
fun Activity.kauSwipeOnDestroy() = SwipeBackHelper.onDestroy(this)

/**
 * Helper function for activities to animate the finish transaction with a pseudo swipe The activity
 * will automatically be finished afterwards
 */
