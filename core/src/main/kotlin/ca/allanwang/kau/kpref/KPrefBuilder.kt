...
interface KPrefBuilder {
  ...
  fun KPref.kpref(
      ...
      fallback: Double,
      postSetter: (value: Float) -> Unit = {}
  ): KPrefDelegate<Float> = kpref(key, fallback.toFloat(), postSetter)
...
class KPrefBuilderAndroid(val sp: SharedPreferences) : KPrefBuilder {
  ...
  override fun KPref.kpref(key: String, fallback: Boolean, postSetter: (value: Boolean) -> Unit) =
      KPrefDelegateAndroid(
          key, fallback, this, this@KPrefBuilderAndroid, KPrefBooleanTransaction, postSetter)

  override fun KPref.kpref(key: String, fallback: Float, postSetter: (value: Float) -> Unit) =
      KPrefDelegateAndroid(
          key, fallback, this, this@KPrefBuilderAndroid, KPrefFloatTransaction, postSetter)

  override fun KPref.kpref(key: String, fallback: Int, postSetter: (value: Int) -> Unit) =
      KPrefDelegateAndroid(
          key, fallback, this, this@KPrefBuilderAndroid, KPrefIntTransaction, postSetter)

  override fun KPref.kpref(key: String, fallback: Long, postSetter: (value: Long) -> Unit) =
      KPrefDelegateAndroid(
          key, fallback, this, this@KPrefBuilderAndroid, KPrefLongTransaction, postSetter)
  ...
  override fun KPref.kpref(
      ...
      KPrefDelegateAndroid(
          key, fallback, this, this@KPrefBuilderAndroid, KPrefSetTransaction, postSetter)

  override fun KPref.kpref(key: String, fallback: String, postSetter: (value: String) -> Unit) =
      KPrefDelegateAndroid(
          key, fallback, this, this@KPrefBuilderAndroid, KPrefStringTransaction, postSetter)
  ...
  override fun KPref.deleteKeys(keys: Array<out String>) {
    // Remove pref listing
    sp.edit().apply { keys.forEach { remove(it) } }.apply()
    // Clear cached values
