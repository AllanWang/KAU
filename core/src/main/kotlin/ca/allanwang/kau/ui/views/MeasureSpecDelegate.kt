 ...
 *
 * Handles relative sizes for any view You may delegate all methods to [MeasureSpecDelegate] and
 * call the two methods: [initAttrs] and [onMeasure]
 */
interface MeasureSpecContract {

  /**
   * Width will be calculated as a percentage of the parent This takes precedence over relativeWidth
   */
  var relativeWidthToParent: Float
  /**
   * Height will be calculated as a percentage of the parent This takes precedence over
   * relativeHeight
   */
  ...
  /** Width will be once again calculated from the current measured height This is the last step */
  var postRelativeWidth: Float
  /** Height will be once again calculated from the current measured width This is the last step */
  var postRelativeHeight: Float

  /** Retrieves relative values from the [AttributeSet] Call this on init */
  fun initAttrs(context: Context, attrs: AttributeSet?)

  /**
   * Calculates the final measure specs Call this from [View.onMeasure] and send the Pair result as
   * the specs The pair is of the format (width, height)
   *
   * Example: <pre> {@code override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
   * ```
