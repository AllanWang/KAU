...
/**
 * Generates a random opaque color Note that this is mainly for testing Should you require this
 * method often, consider rewriting the method and storing the [Random] instance rather than
 * generating one each time
 */
...
fun Int.isColorDark(minDarkness: Float): Boolean =
    ((0.299 * Color.red(this) + 0.587 * Color.green(this) + 0.114 * Color.blue(this)) / 255.0) <
        minDarkness

fun Int.toHexString(withAlpha: Boolean = false, withHexPrefix: Boolean = true): String {
  val hex =
      if (withAlpha) String.format("#%08X", this) else String.format("#%06X", 0xFFFFFF and this)
...
private inline fun Int.colorFactor(rgbFactor: (Int) -> Float): Int {
  val (red, green, blue) =
      intArrayOf(Color.red(this), Color.green(this), Color.blue(this)).map { rgbFactor(it).toInt() }
...
fun Context.textColorStateList(@ColorInt color: Int): ColorStateList {
  ...
  val colors =
      intArrayOf(
          resolveColor(R.attr.colorControlNormal), resolveColor(R.attr.colorControlNormal), color)
...
fun Toolbar.tint(@ColorInt color: Int, tintTitle: Boolean = true) {
  ...
  (0 until childCount).asSequence().forEach {
    (getChildAt(it) as? ImageButton)?.setColorFilter(color)
  }
}
