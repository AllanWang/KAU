...
/**
 * Helper class to launch an activity from a context Counterpart of [Context.startActivity] For
 * starting activities for results, see [startActivityForResult]
 */
 ...
 * Starts a url.
 *
 * If given a series of links, will open the first one that isn't null. Returns [true] if link was
 * opened.
 */
 ...
 *
 * Mainly handles invalid creations, such as showing a dialog when an activity is finishing See
...
/**
 * Check if given context is finishing. This is a wrapper to check if it's both an activity and
 * finishing As of now, it is only checked when tied to an activity
 */
