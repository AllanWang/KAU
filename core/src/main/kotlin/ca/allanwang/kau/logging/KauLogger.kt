 ...
 *
 * Base logger class with a predefined tag This may be extended by an object to effectively replace
 * [Log] Only direct lazy logging is supported, as for best results, applications should extend this
 * and use const/final flags to decide whether logging occurs That way, it will be stripped away by
 * proguard
 *
 * Generally speaking, verbose log may contain private information, as it should be stripped away
 * from production build
 *
 * Debug and info logs may contain sensitive info, and may be differentiated by creating a method
 * such as inline fun _d(message: () -> Any?) {
 * ```
 *      if (BuildConfig.DEBUG) d(message)
 * ```
 * } This use case allows for a constant boolean check, which should be caught and removed by
...
/**
 * Tag extender for [KauLogger] Will prepend [tag] to any expected log output by [logger] Note that
 * if the parent logger is disabled, the extension logger will not output anything either
 */
